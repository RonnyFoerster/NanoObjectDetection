...
| description: size of a camera pixel in object coordinate. This is given by the pixel pitch of the CCD and the 
magnification of the system. Be aware that the magnification written on an objective is only valid for the correct 
tube lens length.
| example: 0.63
| unit: µm/px
...
| description: If a json file is incomplete, for example due to an update, the json file is completed by the default 
value, whose path defined in here.
...
| key: Max_traj_length
| description: Maximum trajectory length. Longer trajectories are cutted and treates as independent particles at 
...
| description: estimated particle size of an object on the camera. This is required for Trackpy to find it with high 
accuaray. The help routine is of use if unknown		
| example: [9.0, 9.0]
| unit: px
...
| key: Dark time
| description: An object can disappear from on frame to another. A reason can be local intensity fluctuations, for 
example generated by large stationary objects. The dark time define how long a object can be invisible before defined 
as lost.
| example: 5
| unit: frames
...
| description: Maximum movement of a particle that is tested between two frames for beeing stationary. 
Important: this check musst happen before the drift correction, because stationary objects are independent 
from the drift and vice versa.
| example: 0.3
| unit: px
    
| key: Min tracking frames before drift
| description: Minimum length of a trajectory before the drift correction. A too short trajectory is considered
of beeing wrong assigned or very noise
| example: 15
| unit: frames
    
| key: Min_tracking_frames
| description: In order to get a good statistic, the squared deviation must be measured several times. This the 
trajectory lenght must have minium number of frames.
| example: 500
| unit: frames
...
| description: Stationary objects are bad, because they are normally quite large and interfere with neighbouring 
particles, which distracts their localzation. Thus, a maximum distance from moving to stationary objects is 
defined in pixels.
bright particle cluster)
| example: 20
| unit: px
...
| description: If a trajectory is cutted, there will be a left over. This 'tail' can be kept, so that one 
trajectore is longer (1), or can be discarded (0) so that all trajectories are of equal length.
| example: 1
| unit: boolean
    
| key: Max rel median intensity step
| description: Defines the maximum relativ intensity drop/raise are trajectory can have between to neighbouring pixels.
 If the value is exceeded the risk of a wrong assignment is too large. Thus the trajectory is splitted at this jump. 
| example: 0.5
| unit: 

| key: ParticleWithLongestTrajBeforeSplit
| description: Particle ID with the longest trajectory. The longest trajectory can be cutted into the most
subtrajectories, which can be all evaluated independently, which gives something like a std/ resolution of the
...
| key: Amount summands
| description: Minimum number of statistically indepent values at each considered lagtime. If a particles does not 
fulfill this criterion, it is not evaluated further. The higher the value the better the fit.
| example: 30
| unit: 
...
| description: Boolean if hindrance factor shall be considered
(good when particle diameter in the range of the fiber diameter)
| example: 1
| unit: boolean
